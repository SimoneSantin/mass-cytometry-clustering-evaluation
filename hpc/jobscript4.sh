...
cp $PBS_O_WORKDIR/dbscan.py $TMPDIR/ # USA IL NUOVO NOME DELLO SCRIPT
...
python dbscan.py $TASK_ID
echo "step 8: Esecuzione Python terminata"


# --- 4. Copia Risultati (USA LA NUOVA VARIABILE TASK_ID) ---
cp dbscan_results_${TASK_ID}.csv $VSC_HOME/dbscan_array_results_${TASK_ID}.csv
